...
export default function RatingForm({ consultationId, smeId, investigatorId }: Props) {
  ...
  if (submitted) return (
    <div className="rounded-xl p-5 mt-4 text-center" style={{ background: 'rgba(255,153,0,0.05)', border: '1px solid rgba(255,153,0,0.2)' }}>
      <Star className="w-8 h-8 mx-auto mb-2" style={{ color: '#f59e0b', fill: '#f59e0b' }} />
      <p className="font-medium" style={{ color: 'var(--text)' }}>Thanks for your feedback!</p>
      <p className="text-sm mt-1" style={{ color: 'var(--text-muted)' }}>Your rating helps improve the consultation process.</p>
    </div>
  )

  return (
    <div className="rounded-xl p-5 mt-4" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
      <h3 className="font-semibold mb-1" style={{ color: 'var(--text)' }}>Rate this consultation</h3>
      <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>How helpful was the SME&apos;s response?</p>
      <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {[1,2,3,4,5].map((star) => (
            <button key={star} type="button" onClick={() => setRating(star)}
              ...
              <Star className="w-8 h-8 transition-colors"
                style={{ color: star <= (hover || rating) ? '#f59e0b' : 'var(--border)', fill: star <= (hover || rating) ? '#f59e0b' : 'var(--border)' }} />
            </button>
          ))}
        </div>
        <div>
          <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-dim)' }}>
            Feedback <span style={{ color: 'var(--text-muted)' }}>(optional)</span>
          </label>
        ...
        <button type="submit" disabled={rating === 0 || loading}
          ...
          style={{ background: 'rgba(245,158,11,0.2)', border: '1px solid rgba(245,158,11,0.4)', color: '#D97706' }}>
