  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>
            <span className="font-medium" style={{ color: '#16A34A' }}>{availableCount} available</span>
            {' · '}{filteredAdvisors.length} total RADAR Advisors
          </p>
        </div>
        <div className="rounded-lg px-4 py-2 text-sm font-medium"
          style={{ background: 'rgba(6,182,212,0.1)', border: '1px solid rgba(6,182,212,0.25)', color: '#0E7490' }}>
      ...
      <div className="rounded-xl p-4 mb-6 flex items-start gap-3"
        ...
        <div>
          <p className="text-sm font-semibold" style={{ color: '#0E7490' }}>About RADAR Advisors</p>
        ...
        {['', 'Available', 'Busy', 'Away'].map((s) => (
          <Link key={s}
            ...
            style={status === s || (!status && !s)
              ? { background: 'rgba(6,182,212,0.2)', color: '#0E7490', border: '1px solid rgba(6,182,212,0.4)' }
      ...
      {filteredAdvisors.length === 0 ? (
        <div className="rounded-xl p-12 text-center" style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)' }}>
          ...
          <p className="font-medium" style={{ color: 'var(--text-muted)' }}>No RADAR Advisors found</p>
          ...
          {filteredAdvisors.map((advisor) => {
            ...
            return (
              <div key={advisor.id} className="rounded-xl p-5 transition-all card-hover"
                style={{ background: 'var(--bg-surface)', border: '1px solid rgba(6,182,212,0.15)' }}>
                {/* Avatar + status */}
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-sm" style={{ color: 'var(--text)' }}>{advisor.full_name}</p>
                  ...
                  <div className="flex flex-wrap gap-1 mb-3">
                    {specializations.slice(0, 3).map((spec: string) => (
                      <span key={spec} className="text-xs px-2 py-0.5 rounded-full"
                        style={{ background: 'rgba(6,182,212,0.08)', color: '#0E7490', border: '1px solid rgba(6,182,212,0.15)' }}>
                        {spec}
                      </span>
                    ))}
