...
export default function SchedulePage() {
  ...
  const statusOptions = [
    { value: 'Available' as AvailabilityStatus, label: '🟢 Available', style: { background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }, activeStyle: { background: 'rgba(22,163,74,0.1)', border: '1px solid rgba(22,163,74,0.4)', color: '#16A34A' } },
    { value: 'Busy' as AvailabilityStatus, label: '🟡 Busy', style: { background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }, activeStyle: { background: 'rgba(217,119,6,0.1)', border: '1px solid rgba(217,119,6,0.4)', color: '#D97706' } },
    { value: 'Away' as AvailabilityStatus, label: '⚫ Away', style: { background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }, activeStyle: { background: 'rgba(100,116,139,0.1)', border: '1px solid rgba(100,116,139,0.4)', color: '#64748B' } },
    { value: 'Off' as AvailabilityStatus, label: '🔴 Off', style: { background: 'var(--bg-surface)', border: '1px solid var(--border)', color: 'var(--text-muted)' }, activeStyle: { background: 'rgba(220,38,38,0.1)', border: '1px solid rgba(220,38,38,0.4)', color: '#DC2626' } },
  ]
  ...
  const toggleBtn = (active: boolean): React.CSSProperties => active
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold" style={{ color: 'var(--text)' }}>My Schedule & Availability</h1>
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>Set when you are available for consultations</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className={sectionClass} style={sectionStyle}>
          <h2 className="font-semibold flex items-center gap-2" style={{ color: 'var(--text)' }}>
            <Globe className="w-4 h-4" style={{ color: 'var(--text-muted)' }} /> Timezone & Hours
          </h2>
          <div>
            <label className="block text-sm font-medium mb-1.5" style={{ color: 'var(--text-dim)' }}>Timezone</label>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[{ label: 'Shift Start', val: shiftStart, set: setShiftStart }, { label: 'Shift End', val: shiftEnd, set: setShiftEnd }].map((f) => (
              <div key={f.label}>
                <label className="block text-sm font-medium mb-1.5 flex items-center gap-1" style={{ color: 'var(--text-dim)' }}>
        ...
        {/* Working days */}
        <div className={sectionClass} style={sectionStyle}>
          <h2 className="font-semibold" style={{ color: 'var(--text)' }}>Working Days</h2>
        ...
        {/* Specializations */}
        <div className={sectionClass} style={sectionStyle}>
          <h2 className="font-semibold" style={{ color: 'var(--text)' }}>Specializations</h2>
          <div className="flex flex-wrap gap-2">
            {SPECIALIZATION_OPTIONS.map((spec) => (
              <button key={spec} onClick={() => toggle(specializations, setSpecializations, spec)}
                className="px-3 py-2 rounded-lg text-sm font-medium transition-all"
                style={toggleBtn(specializations.includes(spec))}>
                {spec}
              </button>
            ))}
          </div>
        ...
        {/* Languages */}
        <div className={sectionClass} style={sectionStyle}>
          <h2 className="font-semibold" style={{ color: 'var(--text)' }}>Languages</h2>
          <div className="flex flex-wrap gap-2">
            {LANGUAGE_OPTIONS.map((lang) => (
              <button key={lang} onClick={() => toggle(languages, setLanguages, lang)}
                className="px-3 py-2 rounded-lg text-sm font-medium transition-all"
                style={languages.includes(lang)
                  ? { background: 'rgba(22,163,74,0.1)', border: '2px solid rgba(22,163,74,0.4)', color: '#16A34A' }
                  : { background: 'var(--bg-elevated)', border: '2px solid var(--border)', color: 'var(--text-muted)' }}>
                {lang}
              </button>
            ))}
