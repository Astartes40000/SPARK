...
import { Shield, ArrowRight, Microscope, GraduationCap } from 'lucide-react'

export default function RegisterPage() {
  const [fullName, setFullName] = useState('')
  const [email, setEmail] = useState('')
  const [password, setPassword] = useState('')
  const [role, setRole] = useState<'sme' | 'investigator'>('investigator')
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { value: 'investigator', label: 'Investigator', icon: <Microscope className="w-5 h-5" />, desc: 'Submit consultations' },
                  { value: 'sme', label: 'SME Expert', icon: <GraduationCap className="w-5 h-5" />, desc: 'Provide expert guidance' },
                ].map((opt) => (
                  ...
                  >
                    <div style={{ color: role === opt.value ? '#a855f7' : '#64748b' }} className="mb-1.5">{opt.icon}</div>
